...
def crps_ensemble(y_true, y_sample):
    ...
    diffs = np.abs(y_sample[:, :, None] - y_sample[:, None, :])
...
for n_0 in [50, 100, 150]:
    for fac_s in [1, 2, 5, 10, 15, 20, 30]:
        ...
        Z_source_qm = np.concatenate([np.ones((M_source.shape[0], 1)), M_source], axis=1)
        ...
        Y_tlcqm_target_sam = Y0_sam_arr @ beta_sol   # shape (n0*M,)
        Y_tlcqm_target_sam_mat = Y_tlcqm_target_sam.reshape(len(Y0), N_sam)

        calib_tlcqm = {
            'W1_to_target': wasserstein_1d(Y_tlcqm_target_sam, np.repeat(Y0, N_sam)),
...
res_full.to_csv('./Results/Simulation_Concept_Covariate_'+str(job_id)+'_abla1.csv', index=False)
